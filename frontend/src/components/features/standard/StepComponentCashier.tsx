...
export const StepComponentCashier: React.FC = () => {
  const { 
    amount, currency, paymentMethod, setPaymentMethod, toNextStep,
    sessionData, applySession, paymentToken, setPaymentToken
  } = useProduct();
  ...
  const handleTokenGen = async (activeInstance?: any) => {
    ...
    try {
      ...
      if (response.code === 'APPLY_SUCCESS') setPaymentToken(response.data.paymentToken);
    } catch (err) { console.error(err); }
    ...
    <div className="flex flex-col h-full bg-[#f8f9fb] font-sans relative">
      ...
      <div className="p-4 bg-white border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] z-40">
        {paymentToken ? (
           <button
              onClick={() => toNextStep()}
              className="w-full h-12 font-bold rounded-full bg-emerald-600 text-white shadow-lg shadow-emerald-50 active:scale-95 transition-all flex items-center justify-center gap-2 text-[13px] uppercase tracking-wider"
            >
